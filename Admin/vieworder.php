<?php
    ...
    $result = mysqli_query($conn, "select o.*, u.username as seller_username, c.username as customer_username
                              from orders o
                              join user u on o.uploaded_by = u.userid
                              join user c on $userid = c.userid
...
<?php
    ...
    while($row=mysqli_fetch_assoc($result))
    {
        ...
        $c_username=$row['customer_username'];
        ...
        echo "<br><div class='card' style='width: 14.6rem;'>
                ...
                <div class='card-body'>
                    ...
                    <p style='color:green;font-size: 14px;'><b>Vendor:</b> $s_username (ID: $uploaded_by)<br> <b>Customer:</b> $c_username (ID: $userid)</p>
                    <p style='color:blue;font-size: 14px;'>Order Placed on: $time</p>";                    
                    if($order_status=='R')
                    {
                    ...
                    else if($order_status=='D')
                    {
                        ...
                        <h6 class='text-success' style='display:inline-block; margin-left:2px;'>Order Delivered</h6>"; 
                    }
                    else
                    {
                        echo"<a href='deleteorder.php?orderid=$orderid' class='btn btn-danger'>Cancel Order</a>"; 
